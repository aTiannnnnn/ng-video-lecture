# add simple feedforward network
import torch
...
class BigramModel(nn.Module):
    def __init__(self, embedding_size, head_size, vocab_size, block_size, num_heads):
        ...
        self.sa_head = MultiHeadAttention(num_heads=num_heads, head_size=head_size, embed_size=embedding_size, block_size=block_size)
        self.ffwd = FeedForward(input_size=multihead_size, output_size=multihead_size)
    ...
    def forward(self, inputs, targets=None):
        ...
        x = self.ffwd(x) # (batch_size, block_size, head_size) / (B, T, C)
