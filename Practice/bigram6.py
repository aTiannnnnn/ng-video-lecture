# add transformer block: multi-head attention + feed forward nn 
# deep nn
import torch
...
class Block(nn.Module):
    ...
    def __init__(self, embedding_size, head_size, block_size, num_heads):
        ...
        self.ffwd = FeedForward(input_size=num_heads*head_size, output_size=num_heads*head_size)
        
    def forward(self, x):
        x = self.sa(x) # comunication
        x = self.ffwd(x)
        return x
