# add residue connection for deep network
import torch
...
class Block(nn.Module):
    ...
    def __init__(self, embedding_size, head_size, block_size, num_heads):
        ...
        self.ffwd = FeedForward(input_size=num_heads*head_size, output_size=4*num_heads*head_size)
        
    def forward(self, x):
        x = x + self.sa(x) # need to make sure that the dimension of x and self.sa(x) are the same
        x = x + self.ffwd(x) # need to make sure that the dimension of x and self.ffwd(x) are the same
        return x
